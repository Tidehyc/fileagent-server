...
namespace fileagent
{
    ...
    namespace
    {
        ...
        FileRecord makeFileRecord(const ConnectionGuard &connection)
        {
            ...
            record.summary = connection->value(7) != nullptr ? connection->value(7) : "";
            record.tags = connection->value(8) != nullptr ? connection->value(8) : "";
            record.created_at = connection->value(9) != nullptr ? connection->value(9) : "";
            return record;
        }
    ...
    std::optional<FileRecord> FileDao::findById(std::int64_t id)
    {
        ...
        std::string sql = "SELECT id, user_id, filename, file_hash, file_size, storage_path, status, summary, tags, created_at FROM files WHERE id = " +
    ...
    std::optional<FileRecord> FileDao::findByHash(const std::string &file_hash)
    {
        ...
        std::string sql = "SELECT id, user_id, filename, file_hash, file_size, storage_path, status, summary, tags, created_at FROM files WHERE file_hash = '" +
    ...
    std::vector<FileRecord> FileDao::listByUserId(std::int64_t user_id, int limit, int offset)
    {
        ...
        std::string sql = "SELECT id, user_id, filename, file_hash, file_size, storage_path, status, summary, tags, created_at FROM files WHERE user_id = " +
    ...
    bool FileDao::updateFileMeta(std::int64_t id,
                                  ...
                                  const std::string &tags)
    {
        ...
        std::string sql = "UPDATE files SET summary = '" +
                          connection->escape(summary) + "', tags = '" +
                          connection->escape(tags) + "' WHERE id = " +
                          std::to_string(id);
        return connection->update(sql);
    }
